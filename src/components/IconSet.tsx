      ...
      {items.map((item, index) => (
        ...
        >
          <div
            ...
            className="relative w-[4.5em] h-[4.5em] overflow-hidden [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] cursor-default rounded-[1.25em]"
          >
            <span
            ...
            >
              <img
                src={item.src}
                alt={item.label}
                className="w-[80%] h-[80%] min-w-0 min-h-0 object-contain"
                loading="lazy"
              />
            </span>
          </div>

          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-base opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(0%)]">
            {item.label}
          </span>
        </div>
      ))}
